...
const Alert = ({ alert, removeAl }) => {
    ...
    setTimeout(async () => {
      try {
        const res = await axios.post(
          `${BASE_API_URL}/alerts/mark?id=${alertId}`
        );
        removeAl(alertId);
      } catch (error) {
        console.log(error);
  ...
  return (
    <div className="w-full pt-5">
      <div
        role="alert"
        className={`alert ${backgroundColor} shadow-lg text-white transform transition-all ease-in-out duration-500 ${
          isHiding
            ? 'opacity-0 scale-0'
            : isInitial
            ? 'opacity-0 scale-0'
        ...
        <div>
          <div className="flex flex-row font-bold">
            ...
            <span className="flex flex-row text-lg">
              <span className=" pl-5 pt-[5px]">
                <FaRegClock />{' '}
              </span>
              {convertTimestamp(alert.timestamp)}
            </span>
          </div>
