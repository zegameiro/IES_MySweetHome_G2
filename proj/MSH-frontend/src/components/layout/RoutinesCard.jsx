...
const RoutineCard = ({ routine, isSensorRoutine }) => {
    ...
    const deleteRoutine = async () => {
        ...
        try {
            if (isSensorRoutine)
                res = await axios.delete(`${BASE_API_URL}/routines/deleteSB?id=${routine.id}`);
            else
                res = await axios.delete(`${BASE_API_URL}/routines/deleteTB?id=${routine.id}`);
        ...
        if (routine) {
            if (isSensorRoutine) {
                if (routine.trigger_type === "range") {
                    return <p className="text-2xl">If the 
                        <span className="font-semibold"> {inputDevice? inputDevice.reading_type : null} reaches </span>
                        <span className="font-semibold">{routine.input_ranges[0]} {inputDeviceUnit ? inputDeviceUnit : null}</span>  then the output device <span className="font-semibold"> {outputDevice ? outputDevice.name : null}</span> should <span className="font-semibold">{routine.associated_action.action_description}  {roomName ? "in the " + roomName : null}</span> until it reaches <span className="font-semibold">{routine.input_ranges[routine.input_ranges.length - 1]} {inputDeviceUnit ? inputDeviceUnit : null}</span> </p>;
                ...
                return <p className="text-2xl">At 
                        <span className="font-semibold"> {convertTimestamp(routine.trigger_timestamp)} </span>
                        the output device <span className="font-semibold"> {outputDevice ? outputDevice.name : null}</span> should <span className="font-semibold">{routine.associated_action.action_description} {roomName ? " in the " + roomName : null}</span> </p>;
            }
        }
        ...
        <div className="pt-[30px]">
            <div className={`card w-[100%] h-[100%] border border-[3px] ${isChecked ? "border-primary" : "border-accent"}  flex flex-col p-4 hover:shadow-xl transition-shadow duration-300`}>
                ...
                <div className="pl-4 pr-4 pb-2">
                    <div className={`badge ${isChecked ? "badge-primary" : "badge-accent"} text-white text-xl font-bold`}>{routine.routine_name !== null ? routine.routine_name : "Routine"}</div>
                    {getRoutineDescription()}
                </div>
                <div className="flex flex-row justify-end space-x-5">
                    <button className="btn btn-error text-white text-lg" onClick={(() => deleteRoutine())}> <FaTrashAlt/> Delete</button>
